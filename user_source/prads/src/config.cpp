...
{
    DEBUG_APP("[*] OS checks enabled:");
    if (IS_COSET(conf,CO_SYN))    DEBUG_APP (" SYN");
    if (IS_COSET(conf,CO_SYNACK)) DEBUG_APP (" SYNACK");
    if (IS_COSET(conf,CO_RST))    DEBUG_APP (" RST");
    if (IS_COSET(conf,CO_FIN))    DEBUG_APP (" FIN");
    if (IS_COSET(conf,CO_ACK))    DEBUG_APP (" ACK");
    DEBUG_APP("\n");
    
    DEBUG_APP("[*] Service checks enabled:");
    if (IS_CSSET(conf,CS_TCP_SERVER))    DEBUG_APP (" TCP-SERVER");
    if (IS_CSSET(conf,CS_TCP_CLIENT))    DEBUG_APP (" TCP-CLIENT");
    if (IS_CSSET(conf,CS_UDP_SERVICES))  DEBUG_APP (" UDP-SERVICES");
    if (IS_CSSET(conf,CS_ICMP))          DEBUG_APP (" ICMP");
    if (IS_CSSET(conf,CS_ARP))           DEBUG_APP (" ARP");
    if (IS_CSSET(conf,CS_MAC))           DEBUG_APP (" MAC");
    DEBUG_APP("\n");
