 ...
 * 				: application name from a signature
 *              : based off of the signature.
 * INPUT        : 0 - Signature Pointer
 * RETURN       : processed app name
 * ---------------------------------------------------------- */
bstring get_app_name_from_sig(signature * sig)
{
    char app[5000];
    bstring retval;
    ...
    if (sig->title.app != NULL) {
        strncpy(app, bdatae(sig->title.app, ""), MAX_APP);
    }

    if (sig->title.ver != NULL) {
        if (sig->title.ver->slen > 0) {
            strcat(app, " ");
            strncat(app, (char *)bdatae(sig->title.ver, ""), MAX_VER);
        }
    }
    if (sig->title.misc != NULL) {
        if (sig->title.misc->slen > 0) {
            strcat(app, " (");
            strncat(app, (char *)bdatae(sig->title.misc, ""), MAX_MISC);
            strcat(app, ")");
        }
    }
	
	retval = bfromcstr(app);
    return retval;
}
