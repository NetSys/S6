...
void service_tcp4(packetinfo *pi, signature* sig_serv_tcp)
{
    ...
    while (tmpsig != NULL) {
        ...
        if (rc >= 0) {
            ...
            pi->cxt->set_check(CXT_SERVICE_DONT_CHECK);
            bdestroy(app);
            return;
        }
        /*
...
/* From here FOR S6 TEST !!! RAndomly found signature */
	...
	while (tmpsig != NULL) {
		if (count == idx) {
            ...
            //printf("[*] - MATCH SERVER IPv4/TCP: %s %s\n",
			//		(char *)bdata(tmpsig->service),
			//		(char *)bdata(app));
			if (pi->asset != nullptr)
				pi->asset->update_asset_service(pi->s_port, pi->proto, pi->sc, 
    ...
    if ( !ISSET_SERVICE_UNKNOWN(pi->cxt->get_check())
		...
		if (pi->asset != nullptr)
			pi->asset->update_asset_service(pi->s_port, pi->proto, pi->sc, -1, 
...
void service_tcp6(packetinfo *pi, signature* sig_serv_tcp)
{
    ...
    if ( !ISSET_SERVICE_UNKNOWN(pi->cxt->get_check())
        && (service_name = check_known_port(IP_PROTO_TCP,ntohs(pi->s_port))) !=NULL ) {
        printf("[*] - MATCH SERVICE IPv6/TCP Port: %s\n",(char *)bdata(service_name));
		if (pi->asset != nullptr)
			pi->asset->update_asset_service(pi->s_port, pi->proto, pi->sc, -1, 
					pi->pheader->ts.tv_sec);
		else
